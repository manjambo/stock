...
class InMemoryOrderRepository : OrderRepository {
    ...
    override fun findByTableNumber(tableNumber: Int, pageable: org.springframework.data.domain.Pageable): org.springframework.data.domain.Page<Order> {
        val filtered = orders.values.filter { it.tableNumber == tableNumber }
        return org.springframework.data.domain.PageImpl(
            filtered.drop(pageable.offset.toInt()).take(pageable.pageSize),
            pageable,
    ...
    override fun findActiveOrders(pageable: org.springframework.data.domain.Pageable): org.springframework.data.domain.Page<Order> {
        val activeStatuses = setOf(
            OrderStatus.PENDING,
            OrderStatus.IN_PROGRESS,
            OrderStatus.READY,
        ...
        val filtered = orders.values.filter { it.status in activeStatuses }
        return org.springframework.data.domain.PageImpl(
            filtered.drop(pageable.offset.toInt()).take(pageable.pageSize),
            pageable,
            filtered.size.toLong()
